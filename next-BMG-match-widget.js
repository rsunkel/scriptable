...
async function createWidget() {
    ...
    if (data.matches[0].homeTeam.id != myTeamId)
        enemyTeamId = data.matches[0].homeTeam.id
    else
        enemyTeamId = data.matches[0].awayTeam.id
        
  
    // some date declaration for further requests
    let dateToday = new Date()
    let monthTo = dateToday.getMonth()+1
    
    if (monthTo < 10)
        monthTo = '0'+monthTo
    
    let monthFrom = dateToday.getMonth()
    ...
    let dateTo = ''+dateToday.getFullYear()+'-'+monthTo+'-'+date
    let dateFrom =''+dateToday.getFullYear()+'-'+monthFrom+'-'+date
    ...
    if(widgetMode == 'Detail' && !config.runsInWidget){
        ...
        let apiRequestPlayedMatchesEnemyTeam = 'http://api.football-data.org/v2/teams/'+enemyTeamId+'/matches?status=FINISHED&dateFrom='+dateFrom+'&dateTo='+dateTo
        ...
        if (playedMatchesEnemyTeamData.errorCode){
            list.addText (apiRequestPlayedMatchesEnemyTeam+  data.message)
            return list
        }
